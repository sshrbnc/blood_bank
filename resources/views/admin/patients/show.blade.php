...
@section('content')
    ...
    <a href="{{ route('admin.patients.newBloodRequests', ['id'=> ($patient->id)] ) }}" class="btn btn-primary" style="background-color: #026C76; border: none;"><i class="fas fa-plus"></i>&nbsp; New Blood Request</a>
    <p></p>
        ...
        <table class="table table-bordered table-striped table-responsive">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    @elseif ($br->status=="Donor donated")
                        <td>{{App\Donor::where( ['id'=>App\Blood::find($br->blood_id)->donor_id] )->pluck('firstname')[0]}} {{App\Donor::where( ['id'=>App\Blood::find($br->blood_id)->donor_id] )->pluck('middlename')[0]}} {{App\Donor::where( ['id'=>App\Blood::find($br->blood_id)->donor_id] )->pluck('lastname')[0]}}</td>
                     @elseif ($br->status=="Donor Assigned")
                        <td>{{App\Donor::where( ['id'=>App\Blood::find($br->blood_id)->donor_id] )->pluck('firstname')[0]}} {{App\Donor::where( ['id'=>App\Blood::find($br->blood_id)->donor_id] )->pluck('middlename')[0]}} {{App\Donor::where( ['id'=>App\Blood::find($br->blood_id)->donor_id] )->pluck('lastname')[0]}}</td>
                    @endif
                </tr>
